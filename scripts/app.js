{
  ...
  const fetchStreetNames = () => {
    ...
    fetch(queryURL)
      ...
      .then(data => {
        ...
        const randomLetter = getRandomProperty(formatedData)
        const randomStreet = getRandomProperty(randomLetter)
        fetchStreetDetails(randomStreet.streetURLName, randomStreet.streetURLId)
        displayStreetNames(formatedData)
      })
      .catch(err => {
        console.log(err)
      })
  ...
  const displayStreetNames = data => {
    for (let i in data) {
      const li = document.createElement('li')
      li.innerHTML = i
      li.id = i.toLowerCase()
      streetsOverview.appendChild(li)
      for (let j in data[i]) {
        ...
        li.appendChild(a)
        streetsOverview.appendChild(li)
        if (j === Object.keys(data[i])[0]) {
          li.classList.add('first')
        }
        a.addEventListener('click', () => {
          fetchStreetDetails(data[i][j].streetURLName, data[i][j].streetURLId)
        })
      }
    }
  }
